/*
严蔚敏《数据结构》
依次读入表达式的每个字符，表达式须以'#'结尾，若是操作数则入栈opval，
若是运算符,则将此运算符c与opter的栈顶元素top比较优先级后执行相应的操作，具体操作如下：
(i)若top的优先级小于c，即top<c，则将c直接入栈opter，并读入下一字符赋值给c；
(ii)若top的优先级等于c，即top=c，则弹出opter的栈顶元素，并读入下一字符赋值给c，这一步目的是进行括号操作;
（iii）若top优先级高于c，即top>c，则表明可以计算，此时弹出opval的栈顶两个元素，并且弹出opter栈顶的的运算符，
计算后将结果放入占opval中。直至opter的栈顶元素和当前读入的字符均为'#',此时求值结束。
*/
